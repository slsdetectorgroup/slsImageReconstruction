...
int main(int argc, char *argv[]) {
    ...
    hid_t datatype, dataspace, vspace, dataprop, dataset;
    hsize_t dim[3] = {(hsize_t)Nimagesexpected - 1 - fileFrameIndex,
                      (hsize_t)((longedge_x == 1) ? npix_y_g : npix_x_g),
                      (hsize_t)((longedge_x == 1) ? npix_x_g : npix_y_g)};
    hsize_t dim2[2] = {dim[1], dim[2]};
    hsize_t vdims[3] = {(hsize_t)imgs, dim[1], dim[2]};
    ...
    // std::vector<std::unique_pointer<uint>> mapContainer;// = new uint[npix_x_g * npix_y_g];
    uint **mapContainer = new uint *[Nimagesexpected];
    uint **maprContainer = new uint *[Nimagesexpected];
    std::vector<std::vector<unsigned int *>> bufferContainer;

    for (int iframes = 1; iframes < Nimagesexpected; iframes++) {
        mapContainer[iframes - 1] = new uint[npix_x_g * npix_y_g];
        maprContainer[iframes - 1] = new uint[npix_x_g * npix_y_g];
        vec<unsigned int *> buffer;
        buffer.reserve(n_v * n_h * 2 * 2 /**Nimgscashed*/);
...
#pragma omp parallel for
    for (int iframes = 1; iframes < Nimagesexpected; iframes++) {
        ...
        uint *map = mapContainer[iframes - 1];
        uint *mapr = maprContainer[iframes - 1];

        std::vector<unsigned int *> buffer = bufferContainer[iframes - 1];
                    ...
                    for (int ichipy = 0; ichipy < 2; ++ichipy) {
                        for (int iy = 0; iy < NumChanPerChip_y; ++iy) {
                            ...
                            if (ichipy == 1 && iy == 0) {
                                ...
                                if (ichipy == 0 && iy == NumChanPerChip_y - 1)
	 ...
	 */
        ...
        start[0] = /*nf[ifiles]*/ iframes - 1 - fileFrameIndex;
        // hid_t dataspaceimg = H5Screate_simple(2, dim2, NULL);
        ...
        } else if (dynamicrange == 8 || dynamicrange == 4) {
    ...
    for (int iframes = 1; iframes < Nimagesexpected; iframes++) {
        delete[] mapContainer[iframes - 1];
        delete[] maprContainer[iframes - 1];
    }
    delete[] mapContainer;
    delete[] maprContainer;
    for (int inr = 0; inr < nr; ++inr)
