...
int main(int argc, char *argv[]) {
  ...
  uint* map2=new uint[npix_x_g*npix_y_g]; //sum
  ...
  TFile* ofile = new TFile(TString::Format("%s_Sum%s_%d.root",file.c_str(),
	...
	if(runningFrameIndex==1) //initialize only once
	  for(int ik=0; ik<npix_y_g*npix_x_g;++ik)
	    map2[ik]=0;
	...
	if(runningFrameIndex==Nimgs) {
#ifdef MYCBF
	
	//---> here I should also fill
	/* Create and initializes new internal CBF Object*/
	cbf_failnez (cbf_make_handle (&cbf));
	sprintf(fname, "%s_Sum_%05d_%05d.cbf",file.c_str(),fileIndex, numFrames/Nimgs);
	out = fopen (fname, "w");
	...
	cbf_failnez (cbf_new_datablock (cbf, "image_1"))  //why not: cbf_new_saveframe(cbf,"image 1")
	  ...
	  cbf_failnez (cbf_set_integerarray_wdims_fs (
						      ...
						      1,									//int binary_id
						      ( longedge_x ? &(map2[0]) : &(mapr[0])), 						//void *array
...
#endif  //HDF5f
	} // //write the cbf file <-----this is important
	//   if(im==readim-1)
	//delete[] longbuffer;
