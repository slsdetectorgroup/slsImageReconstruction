...
int getCommandParameters(int argc, char *argv[], string &file, int &fileIndex, bool &isFileFrameIndex, int &fileFrameIndex, int &npix_x_user, int &npix_y_user, int& Nimgs, int& longedge_x, int &startdet);
...
int main(int argc, char *argv[]) {
  ...
  int Nimgs=1; 
  getCommandParameters(argc, argv, file, fileIndex, isFileFrameIndex, fileFrameIndex, npix_x_user, npix_y_user, Nimgs, longedge_x,startdet);
  ...
  while(fnum[0]>-1){
	...
	if(!CheckFrames(fnum[inr],runningFrameIndex)) continue;
		...
		if( npix_y_user!=256 ){			      
		  for(int ichipy=1; ichipy<NumChip_y;ichipy++){
			...
			for(int ix=0; ix<NumChanPerChip_x;ix++){
			  ...
			  map2[k]=(receiverdata[inr]->getValue(buffer.at(inr),
...
int  getCommandParameters(int argc, char *argv[], string &file, int &fileIndex, bool &isFileFrameIndex, int &fileFrameIndex, int &npix_x_user, int &npix_y_user,int& Nimgs, int& longedge_x, int &startdet){
  ...
  if (sscanf( s.substr(uscore+1,s.size()-uscore-1).c_str(),"f%d",&i)) {
    ...
    if(argc>2){
      if(argc < 4){
	cprintf(RED, "Error: Not enough arguments: cbfMakerSum [file_name_with_dir] "  "  NimgsToSum"
